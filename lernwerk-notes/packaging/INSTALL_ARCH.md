...
```bash
chmod +x FaNotes-2026.7.4-beta.1-x86_64.AppImage
./FaNotes-2026.7.4-beta.1-x86_64.AppImage
```
...
install -m755 FaNotes-2026.7.4-beta.1-x86_64.AppImage \
...
```bash
./FaNotes-2026.7.4-beta.1-x86_64.AppImage --appimage-extract-and-run
```
...
tar -xzf FaNotes-2026.7.4-beta.1-x86_64.tar.gz -C "$HOME/.local/opt"
ln -sfn \
  "$HOME/.local/opt/FaNotes-2026.7.4-beta.1-x86_64/fanotes" \
  "$HOME/.local/bin/fanotes"
"$HOME/.local/bin/fanotes"
```
...
rm -rf "$HOME/.local/opt/FaNotes-2026.7.4-beta.1-x86_64"
```
...
1. `FaNotes-2026.7.4-beta.1-x86_64.tar.gz`, `PKGBUILD`, `fanotes.desktop`, `fanotes.svg` und `LICENSE` gemeinsam in einen leeren Arbeitsordner kopieren.
...
- Zeichnungen: `.lernwerk/assets/*.png` und `.lernwerk/assets/*.json` im Vault
- importierte Arbeitsblätter und Textfelder: `.lernwerk/worksheets/*` im Vault
- Einstellungen und Trainingsmodell: Electron-Profil unter `~/.config/FaNotes/`; beim ersten Start werden vorhandene Daten aus `~/.config/Lernwerk Notes/` oder `~/.config/lernwerk-notes/` übernommen
- Papierkorb: Gelöschte Vault-Dateien werden an den Papierkorb der Desktop-Umgebung übergeben

Für ein Backup die App zuerst beenden und Vault plus Profilordner kopieren. Bei Syncthing, Git oder NAS-Synchronisation sollte derselbe Markdown-Text nicht gleichzeitig auf zwei Geräten bearbeitet werden; sonst entscheidet das jeweilige Synchronisationswerkzeug über Konfliktdateien. Der versteckte `.lernwerk`-Ordner gehört zum Vault-Backup.
