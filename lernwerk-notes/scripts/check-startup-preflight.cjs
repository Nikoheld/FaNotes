...
try {
  ...
  assert.match(appSource, /startupBootstrap\s*\?\?\s*window\.lernwerk\.bootstrap\(\)/u, 'Lokale Config und englischer Katalog sollen parallel laden.')
  assert.match(appSource, /cachedTree\s*\?\?\s*await window\.lernwerk\.getFastTree\(\)/u)
