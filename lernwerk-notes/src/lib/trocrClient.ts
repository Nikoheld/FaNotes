...
const createWorker = () => {
  ...
  next.onmessage = (event: MessageEvent<TrocrWorkerResponse>) => {
    ...
    pending.delete(id)
    globalThis.clearTimeout(request.timeout)
    if (typeof event.data.text === 'string') request.resolve(event.data.text)
...
const recognitionAssetRoot = () => window.lernwerk.platform === 'web'
...
const loadVerifiedDesktopRuntimeModule = (root: string) => {
  if (window.lernwerk.platform === 'web') return Promise.resolve<string | undefined>(undefined)
...
export const recognizeTrocrLine = async (
  ...
  const modelId = window.lernwerk.platform === 'web' ? 'fanotes-trocr-web' : 'fanotes-trocr'
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    try {
      createWorker().postMessage({
        ...
        runtimeModuleSource,
        threads: effectiveOcrThreadCount(window.lernwerk.platform === 'web' ? 2 : 4),
      }, [transferable])
    } catch (error) {
      pending.delete(id)
