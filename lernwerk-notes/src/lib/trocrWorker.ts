...
globalThis.onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    ...
    const output = await recognizer(image, {
      max_new_tokens: 160,
      // Two deterministic beams reduce the independent German holdout CER
      // without the steep latency/memory cost of four-way decoding.
      num_beams: 2,
      do_sample: false,
    })
    const first = output[0]
    const generatedText = Array.isArray(first) ? first[0]?.generated_text : first?.generated_text
    const text = normalizeGermanSharpS(generatedText ?? '')
      .normalize('NFC')
      .replace(/[ \t]{2,}/gu, ' ')
      .replace(/\s+([,.;:!?])/gu, '$1')
      .trim()
