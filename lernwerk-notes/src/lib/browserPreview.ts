...
import type { AppSettings, DrawingLibraryDocument, LernwerkApi, UpdateState, VaultEntry, WorksheetDocument } from '../types'
...
export function createBrowserPreviewApi(): LernwerkApi {
  ...
  const updateState: UpdateState = {
    status: 'up-to-date',
    supported: false,
    currentVersion: '2026.7.1-beta.1',
    latestVersion: '2026.7.1-beta.1',
  ...
  return {
    ...
    importWorksheet: async () => {
      ...
      const sourceRelativePath = `.lernwerk/worksheets/${id}.png`
      const dataRelativePath = `.lernwerk/worksheets/${id}.json`
