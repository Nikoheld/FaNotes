...
import type { AppSettings, BootstrapData, DrawingLibraryDocument, LernwerkApi, ServerBackupState, UpdateState, VaultEntry, WorksheetDocument } from '../types'
...
const WEB_VERSION = '2026.7.4-beta.4'
...
export function createBrowserApi(): LernwerkApi {
  ...
  return {
    ...
    createNote: async (rawParent = '', preferredName = english ? 'Untitled Note' : 'Unbenannte Notiz') => {
      await ready
    ...
    saveDrawing: async ({ id = crypto.randomUUID(), title, imageData, drawingJson }) => {
      ...
      const imageRelativePath = `.lernwerk/assets/${id}.png`
      const dataRelativePath = `.lernwerk/assets/${id}.json`
    ...
    importWorksheet: async () => {
      ...
      const now = new Date().toISOString()
      const sourceRelativePath = `.lernwerk/worksheets/${id}.${extension}`
      const document: WorksheetDocument = { schemaVersion: 1, id, title: safeSegment(stem(file.name), 'Arbeitsblatt'), kind: format.kind, mimeType: format.mimeType, sourceRelativePath, dataRelativePath: `.lernwerk/worksheets/${id}.json`, createdAt: now, updatedAt: now, textBoxes: [] }
    ...
    resetAppData: async () => {
      ...
      await new Promise<void>((resolve) => {
        const request = indexedDB.deleteDatabase('lernwerk-notes-handwriting')
