...
const loadDictionaries = async (): Promise<Dictionaries> => {
  dictionariesPromise ??= window.lernwerk.loadSpellingResources().then(async (resources) => {
...
const loadRecognitionCandidates = async (language: SpellingLanguage) => {
  ...
  if (!pending) {
    pending = window.lernwerk.loadSpellingWordCandidates(language).then(async (resource) => {
