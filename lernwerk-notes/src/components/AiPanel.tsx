...
export function AiPanel({ settings, note, vaultNotes, onSettingsChange, onApply, onClose }: {
  ...
  const loadModels = async () => {
    ...
    setResult(null)
    try {
      const available = await window.lernwerk.aiListModels(connection)
      setModels(available)
  ...
  const run = async () => {
    ...
    try {
      const transformed = await window.lernwerk.aiTransform({ connection, title: note.title, relativePath: note.relativePath, markdown: note.markdown, actions: [...actions], instruction, vaultNotes })
