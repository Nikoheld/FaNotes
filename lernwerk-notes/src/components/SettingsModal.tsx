...
export function SettingsModal({
  ...
  useEffect(() => {
    if (!isWeb || !window.lernwerk.getServerBackupState) return
    let activeRequest = true
    window.lernwerk.getServerBackupState().then((state) => { if (activeRequest) setBackupState(state) }).catch((error) => {
  ...
  const runBackupAction = async (action: () => Promise<ServerBackupState>, success: string) => {
    ...
    try {
      ...
      const localState = await window.lernwerk.getServerBackupState?.().catch(() => null)
  ...
  const enableBackup = async () => {
    if (!window.lernwerk.enableServerBackup) return
    const next = await runBackupAction(() => window.lernwerk.enableServerBackup!(backupEnrollmentCode), 'Das erste, vollständig geprüfte Server-Backup wurde gespeichert.')
                ...
                {isWeb && <div id="settings-backup" className="setting-card server-backup-card">
                  ...
                  {!backupState.enabled ? <div className="server-backup-body">
                    ...
                    <div className="backup-recovery-connect">
                      ...
                      <div><input id="server-backup-recovery" type="password" autoComplete="off" spellCheck={false} value={recoveryInput} placeholder="fanotes1_…" onChange={(event) => setRecoveryInput(event.target.value)} /><button type="button" className="secondary-button" disabled={backupBusy || !recoveryInput.trim() || !window.lernwerk.connectServerBackup} onClick={() => void runBackupAction(() => window.lernwerk.connectServerBackup!(recoveryInput), 'Backup-Schlüssel geprüft. Du kannst die Daten nun wiederherstellen.')}>Verbinden</button></div>
                    </div>
