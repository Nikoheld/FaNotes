...
export type PersonalizedTextRecognitionResult = {
  ...
  /** Untrained terminal debris removed after it exposed a complete known word. */
  discardedUnsupportedCharacters?: number
}
...
const personalizedDictionaryWord = (
  ...
  const maximumDistance = allowLongPersonalRepair && lower.length >= 8
    ...
    : allowLongPersonalRepair && lower.length >= 5
      ? 2
    : lower.length >= 6 ? 2 : 1
  const normalNeighbours = nearestNeuralWordContextCandidates(
    lower,
    language,
    maximumDistance,
    // A complete token sequence may contain one stolen or spurious connected
    // piece. Its dynamic visual alignment can safely evaluate a neighbouring
    // word length; ordinary partial paths remain strictly same-length.
    !allowLongPersonalRepair,
  )
  ...
  const evaluated = substitutions.map(({ candidate, distance }) => {
    ...
    candidateCharacters.forEach((character, index) => {
      ...
      const original = sourceCharacter
        ? candidateEvidence(token, sourceCharacter, language)
        : null
    ...
    const safeJointLongCommonRepair = (
      ...
      // A distant common word must never leapfrog a closer exhaustive
      // dictionary neighbour merely because the latter is outside the tiny
      // hand-curated common set. Visual alignment decides between equally
      // close words; four-edit semantic projection is only a last resort.
      !normalNeighbours.some((entry) => entry.distance < distance) &&
...
const cleanUnsupportedTerminalWord = (
  value: string,
  neuralValue: string,
  language: RecognitionLanguage,
  terminalHasPersonalSupport: boolean,
) => {
  if (terminalHasPersonalSupport) return value
  const match = /^(\p{L}{2,})([^\p{L}\s.,;:!?]+)$/u.exec(value)
  if (!match) return value
  const neuralTerminal = /[^\p{L}\s.,;:!?]+$/u.exec(neuralValue)?.[0] ?? ''
  if (neuralTerminal === match[2]) return value
  const locale = language === 'de' ? 'de-CH' : 'en-US'
  const lower = match[1].toLocaleLowerCase(locale)
  return lexicons[language].has(lower) || isExtendedNeuralContextWord(lower, language)
    ? match[1]
    : value
}

export const cleanUnsupportedTerminalWordForTests = cleanUnsupportedTerminalWord
...
const chooseFusedLineSpacing = (
  ...
  const candidates = (['neural', 'token', 'consensus'] as const)
    .map((boundary) => {
      ...
      const unsupportedNeuralSpaces = characters.filter((entry) => (
        entry.neuralSpace && !entry.tokenSpace
      )).length
      const unsupportedTokenSpaces = characters.filter((entry) => (
        entry.tokenSpace && !entry.neuralSpace
      )).length
      // A line model occasionally inserts a fluent-looking space inside a
      // continuous handwritten name or technical term. Previously its generic
      // confidence bonus won every lexical tie, even though no physical word
      // boundary existed. Keep language capable of recovering two clear known
      // words, but make an unsupported boundary pay for contradicting the ink.
      const boundaryDisagreementPenalty = boundary === 'neural'
        ? unsupportedNeuralSpaces * 0.28 + unsupportedTokenSpaces * 0.32
        : boundary === 'consensus'
          ? (unsupportedNeuralSpaces + unsupportedTokenSpaces) * 0.18
          : 0
      const neuralBoundaryAgreement = unsupportedNeuralSpaces === 0
        && unsupportedTokenSpaces === 0
      return {
        text,
        score: wordBoundaryScore(
          text,
          language,
          boundary === 'neural' && neuralLine.confidence >= 52 && neuralBoundaryAgreement,
        ) - boundaryDisagreementPenalty,
      }
    })
...
export const personalizedTextFusionSelectionScore = (
  ...
  const completePersonalLengthRecovery = (
    personalRatio >= 0.9
    && lengthDifference >= 3
    && unsupportedChanges === 0
    && cleanKnownWordRatio >= 0.99
  )
  return (
    fusion.confidence / 100
    // This ratio contains only glyphs that passed the personal/base evidence
    // checks; generic geometric fallbacks are counted separately. Give a
    // mostly personal whole-word segmentation enough weight to beat a fluent
    // neural word supported by only one trained character. Length, lexical and
    // unsupported-change penalties below still prevent arbitrary over-splits.
    + personalRatio * 1.82
    + classicalRatio * 0.12
    + lengthAwareKnownWordRatio * 0.82
    // Explaining an extra generic token as unsupported terminal debris is a
    // stronger segmentation signal than rewriting the complete personal word
    // into another same-length dictionary entry. This bonus is only reachable
    // after the strict known-word/neural/personal guard removed the suffix.
    + Math.min(1, fusion.discardedUnsupportedCharacters ?? 0) * 0.70
    // Repeated syllables are a common whole-line decoder failure. Once a
    // shorter/longer dictionary sequence is independently explained almost
    // entirely by trained glyphs, the raw length gap is evidence against the
    // line hypothesis rather than against the personal sequence.
    + (completePersonalLengthRecovery ? Math.min(1.8, lengthDifference * 0.35) : 0)
...
export const fusePersonalizedTextRecognition = (
  ...
  if (!disableWordwiseFusion) {
    ...
    if (hasMultiwordLine && hasMatchingPhysicalWords) {
      ...
      const discardedUnsupportedCharacters = flat.reduce((sum, fusion) => (
        sum + (fusion.discardedUnsupportedCharacters ?? 0)
      ), 0)
      ...
      return {
        ...
        classicalCharacters,
        unsupportedChanges,
        ...(discardedUnsupportedCharacters ? { discardedUnsupportedCharacters } : {}),
      }
    }
  }
  ...
  const selectedPersonalCoverage = visibleTokens.filter((token) => {
    const selected = selectedPersonalCandidate(token)
    return Boolean(
      selected
      && normalizedCharacter(selected.char, language) === normalizedCharacter(token.char, language)
      && token.confidence >= 56,
    )
  }).length / Math.max(1, visibleTokens.length)
  ...
  const observedSequenceOutvotesLengthHallucination = (
    comparableSequenceLengths &&
    Math.abs(classicalVisibleCharacters - neuralVisibleCharacters) >= 2 &&
    classicalRatio >= 0.99 &&
    observedClassicalRatio >= 0.84 &&
  ...
  const completeSelectedSequenceOutvotesLengthHallucination = (
    comparableSequenceLengths &&
    Math.abs(classicalVisibleCharacters - neuralVisibleCharacters) >= 3 &&
    classicalRatio >= 0.99 &&
    selectedPersonalCoverage >= 0.9 &&
    visibleTokens.length === classicalVisibleCharacters &&
    classicalConfidence >= 0.68
  )
  ...
  const trustedCommonNeuralWordOutvotesOneShot = (
    ...
    // restore that contradicted length hypothesis.
  ...
  let discardedUnsupportedCharacters = 0
  const terminalCharacter = Array.from(text).at(-1) ?? ''
  const terminalToken = visibleTokens.at(-1)
  const terminalEvidence = terminalToken && terminalCharacter
    ? candidateEvidence(terminalToken, terminalCharacter, language)
    : undefined
  const cleanedTerminalText = cleanUnsupportedTerminalWord(
    text,
    measuredNeuralText,
    language,
    Boolean(terminalEvidence && terminalEvidence.personalSupport > 0),
  )
  if (cleanedTerminalText !== text) {
    const removedCharacters = Array.from(text).length - Array.from(cleanedTerminalText).length
    text = cleanedTerminalText
    classicalCharacters = Math.max(0, classicalCharacters - removedCharacters)
    discardedUnsupportedCharacters = removedCharacters
  }
