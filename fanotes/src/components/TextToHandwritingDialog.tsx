...
export function TextToHandwritingDialog({
  ...
  const insertClipboardText = (incoming: string, field: HTMLTextAreaElement) => {
    const start = field.selectionStart ?? text.length
    const end = field.selectionEnd ?? text.length
    const next = `${text.slice(0, start)}${incoming}${text.slice(end)}`.slice(0, 8_000)
    setText(next)
    const cursor = Math.min(8_000, start + incoming.length)
    requestAnimationFrame(() => {
      try { field.setSelectionRange(cursor, cursor) } catch { /* ignore */ }
    })
  }

  const handleDialogPointer = (event: { stopPropagation: () => void }) => {
  ...
  const previewWidth = result.pageWidth || pageWidth
  const previewHeight = result.pageHeight || pageHeight

  return <div
    className="lw-tth-backdrop"
    role="presentation"
    data-fanotes-drawing-chrome="text-to-handwriting"
    onPointerDown={(event) => {
      event.stopPropagation()
      if (event.target === event.currentTarget) onClose()
    }}
    onPointerMove={handleDialogPointer}
    onPointerUp={handleDialogPointer}
    onPaste={(event) => event.stopPropagation()}
  >
    <style>{textToHandwritingStyles}</style>
    <section
      ref={dialogRef}
      className="lw-tth-dialog"
      role="dialog"
      aria-modal="true"
      aria-labelledby="lw-tth-title"
      onKeyDown={handleDialogKeyDown}
      onPointerDown={handleDialogPointer}
    >
      ...
      <div className="lw-tth-body">
        <div className="lw-tth-controls">
          <label className="lw-tth-text-field">
            ...
            <textarea
              ...
              onPaste={(event) => {
                ...
                event.preventDefault()
                event.stopPropagation()
                insertClipboardText(incoming, event.currentTarget)
              }}
            />
          ...
          <div className="lw-tth-option-grid">
            <label>
              <span>Schriftgröße <output data-tth-output="font-size">{fontSize} px</output></span>
              <input
                type="range"
                data-tth-control="font-size"
                min="20"
                max="72"
                step="1"
                value={fontSize}
            ...
            <label>
              <span>Zeilenabstand <output data-tth-output="line-spacing">{lineSpacing.toFixed(2)}×</output></span>
              <input
                type="range"
                data-tth-control="line-spacing"
                min="1.05"
                max="2"
                step="0.01"
                value={lineSpacing}
                onInput={(event) => setLineSpacing(Number(event.currentTarget.value))}
                onChange={(event) => setLineSpacing(Number(event.currentTarget.value))}
              />
            </label>
            <label>
              <span>Natürliche Variation <output data-tth-output="variation">{Math.round(variation * 100)}%</output></span>
              <input
                type="range"
                data-tth-control="variation"
                min="0"
                max="1"
                step="0.01"
                value={variation}
                onInput={(event) => setVariation(Number(event.currentTarget.value))}
                onChange={(event) => setVariation(Number(event.currentTarget.value))}
              />
            </label>
          </div>
        ...
        <aside className="lw-tth-preview">
          <div
            className="lw-tth-preview-head"
            data-tth-preview-options={JSON.stringify({
              fontSize,
              lineSpacing,
              variation,
          ...
          <div className="lw-tth-paper-wrap">
            <HandwritingPreview result={result} pageWidth={previewWidth} pageHeight={previewHeight} paperStyle={paperStyle} />
...
.lw-tth-preview{display:flex;min-height:0;flex-direction:column;padding:20px;border-left:1px solid var(--border,rgba(255,255,255,.1));background:color-mix(in srgb,var(--background-secondary,#191920) 72%,transparent)}.lw-tth-preview-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:11px}.lw-tth-preview-head>div{display:flex;min-width:0;flex-direction:column}.lw-tth-preview-head small{color:var(--text-muted,#999);font-size:10px}.lw-tth-preview-head button{display:flex;height:32px;align-items:center;gap:6px;padding:0 9px;border:1px solid var(--border,rgba(255,255,255,.12));border-radius:8px;color:inherit;background:color-mix(in srgb,var(--text,#fff) 5%,transparent);cursor:pointer}.lw-tth-preview-head button:disabled{opacity:.45;cursor:not-allowed}.lw-tth-paper-wrap{position:relative;display:grid;min-height:310px;flex:1;place-items:center;overflow:auto;border-radius:12px;background:#282832;box-shadow:inset 0 0 0 1px rgba(255,255,255,.05)}.lw-tth-preview-canvas{display:block;width:auto;max-width:calc(100% - 24px);height:auto;max-height:none;background:#fbfcff;box-shadow:0 12px 36px rgba(0,0,0,.3)}.lw-tth-preview-empty{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:9px;color:#8f91a0;font-size:11px;pointer-events:none}.lw-tth-fit-note{margin:9px 0 0;color:var(--text-muted,#999);font-size:10px;text-align:center}
