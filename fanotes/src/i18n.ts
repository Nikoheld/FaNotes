...
function translateDerived(source: string, catalog: EnglishCatalog): string | null {
  ...
  const revertAutocorrect = /^Autokorrektur rückgängig: „(.+)“$/u.exec(source)
  if (revertAutocorrect) return `Undo autocorrect: “${revertAutocorrect[1]}”`
  ...
  const pageTimestamps = /^Erstellt (.+) · Geändert (.+)$/u.exec(source)
  if (pageTimestamps) return `Created ${pageTimestamps[1]} · Modified ${pageTimestamps[2]}`
