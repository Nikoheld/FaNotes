...
const INTERNAL_VAULT_DIRECTORY = '.fanotes'
const LEGACY_INTERNAL_VAULT_DIRECTORY = '.lernwerk'
const PROTECTED_INTERNAL_VAULT_DIRECTORIES = new Set([
  INTERNAL_VAULT_DIRECTORY,
  LEGACY_INTERNAL_VAULT_DIRECTORY,
])
...
async function ensureInternalVaultDirectory(root) {
  const internalDirectory = path.join(root, INTERNAL_VAULT_DIRECTORY)
  const legacyInternalDirectory = path.join(root, LEGACY_INTERNAL_VAULT_DIRECTORY)
  try {
    ...
    let migrated = false
    try {
      const legacyInfo = await fsp.lstat(legacyInternalDirectory)
      if (!legacyInfo.isDirectory() || legacyInfo.isSymbolicLink()) {
        throw new Error('Der bisherige interne FaNotes-Ordner ist unsicher.')
      }
      const realLegacyDirectory = await fsp.realpath(legacyInternalDirectory)
      if (!isInsideRoot(root, realLegacyDirectory)) {
        throw new Error('Der bisherige interne FaNotes-Ordner verlässt den Vault.')
      }
      await fsp.rename(legacyInternalDirectory, internalDirectory)
      migrated = true
    } catch (legacyError) {
      if (legacyError?.code !== 'ENOENT' && legacyError?.code !== 'EEXIST') throw legacyError
    }
    if (!migrated) {
...
async function readOnboardingStatus(root) {
  const internalDirectory = await ensureInternalVaultDirectory(root)
  const statePath = path.join(internalDirectory, 'onboarding.json')
  try {
...
function normalizeRelativePath(value, { allowRoot = false, allowInternal = false } = {}) {
  ...
  if (!allowInternal && PROTECTED_INTERNAL_VAULT_DIRECTORIES.has(segments[0].toLocaleLowerCase('en-US'))) {
...
async function ensureInternalAssetsDirectory() {
  const root = await assertCurrentVaultRoot()
  let cursor = await ensureInternalVaultDirectory(root)
  for (const segment of ['assets']) {
...
async function ensureInternalWorksheetsDirectory() {
  const root = await assertCurrentVaultRoot()
  let cursor = await ensureInternalVaultDirectory(root)
  for (const segment of ['worksheets']) {
    cursor = path.join(cursor, segment)
    try {
      const info = await fsp.lstat(cursor)
...
function validateWorksheetDocument(candidate, expectedId) {
  ...
  const expectedSource = `${INTERNAL_VAULT_DIRECTORY}/worksheets/${id}${extension}`
  const expectedData = `${INTERNAL_VAULT_DIRECTORY}/worksheets/${id}.json`
  const legacyExpectedSource = `${LEGACY_INTERNAL_VAULT_DIRECTORY}/worksheets/${id}${extension}`
  const legacyExpectedData = `${LEGACY_INTERNAL_VAULT_DIRECTORY}/worksheets/${id}.json`
  const usesCurrentPaths = candidate.sourceRelativePath === expectedSource && candidate.dataRelativePath === expectedData
  const usesLegacyPaths = candidate.sourceRelativePath === legacyExpectedSource && candidate.dataRelativePath === legacyExpectedData
  if (
    !format ||
    candidate.kind !== format.kind ||
    candidate.mimeType !== format.mimeType ||
    (!usesCurrentPaths && !usesLegacyPaths)
  ) {
...
function drawingLibraryMetadata(document, id, fileModifiedAt) {
  ...
  return {
    ...
    updatedAt,
    imageRelativePath: `${INTERNAL_VAULT_DIRECTORY}/assets/${id}.png`,
    dataRelativePath: `${INTERNAL_VAULT_DIRECTORY}/assets/${id}.json`,
  }
}
...
function registerIpcHandlers() {
  ...
  handle(IPC.createNote, async (_event, rawParentPath, rawPreferredName) => {
    await ensureBootstrap()
    ...
    const preferredName = typeof rawPreferredName === 'string' && rawPreferredName.trim()
      ? rawPreferredName
      : localizeText('Unbenannte Notiz', currentUiLanguage())
    ...
    for (let attempt = 0; attempt < 10000; attempt += 1) {
      ...
      try {
        ...
        created = true
        await fileHandle.writeFile(`# ${title}\n\n`, 'utf8')
        ...
        if (created) await fsp.rm(absolutePath, { force: true }).catch(() => {})
        if (error?.code !== 'EEXIST') throw error
