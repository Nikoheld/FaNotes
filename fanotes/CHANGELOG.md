...
- **Einklappbare Abschnitte beim Handschreiben.** Im Stiftmodus fügt „Abschnitt“ mit einem Tipp auf das Blatt eine Titelzeile ein — alles darunter rückt nach unten, nichts wird überdeckt. In die Kopfzeile schreibst du den Titel von Hand (etwa „Aufgabe 3“), darunter hat der Abschnitt beliebig viel Platz: Wer nahe an den nächsten Titel schreibt, schiebt ihn samt allem Folgenden weiter nach unten. Der Pfeil am linken Rand klappt den Inhalt ein — die Tinte des Abschnitts verschwindet, der Rest der Seite rückt nach oben — und wieder aus, exakt an die alte Stelle. Abschnitte werden mit der Stiftebene gespeichert; das × in der Kopfzeile löst einen Abschnitt auf und lässt die Tinte liegen. Ein- und Ausklappen setzt den Verlauf von Rückgängig zurück, weil verschobene Striche sonst an falscher Stelle wiederkämen. PDF-Notizen bieten keine Abschnitte, ihre Tinte gehört zur Seite darunter; auf Textnotizen bewegen Abschnitte nur die Handschrift, nicht den getippten Text.
- **Sync: deine Notizen auf allen Geräten, Ende-zu-Ende verschlüsselt.** Unter Einstellungen → Sync (oder Strg+P → „Sync einrichten“) legst du mit E-Mail-Adresse und Passwort ein Konto an; jedes weitere Gerät meldet sich damit an und holt den ganzen Tresor — Notizen, `.famd`-Begleiter, PDFs, Bilder, Ordnerfarben und Fachbücher. Danach läuft der Abgleich von selbst: 2,5 Sekunden nach jeder Änderung, jede Minute, beim Zurückkehren ins Fenster und sobald das Netz wieder da ist; ein Eintrag in der Statusleiste zeigt Stand, Fehler und Konflikte, „Jetzt synchronisieren“ steht in der Palette. Aus dem Passwort wird auf dem Gerät ein Schlüssel abgeleitet (PBKDF2, 600 000 Runden), der einen zufälligen Tresorschlüssel einpackt; jede Datei wird mit AES-256-GCM verschlüsselt, ihr Name durch einen HMAC ersetzt. Der Server speichert nur Chiffrate, zufällige Kennungen, eine Scrypt-Ableitung des Anmeldeschlüssels und die Geräteliste — er kann weder Inhalte noch Dateinamen lesen, und deshalb lässt sich ein vergessenes Passwort auch nicht zurücksetzen. Wird dieselbe Datei auf zwei Geräten geändert, gewinnt die zuerst hochgeladene Fassung den Pfad und die andere bleibt als „(Konflikt Gerät Datum)“-Kopie erhalten; eine offene, ungespeicherte Notiz wird nie von außen überschrieben. Geräte lassen sich einzeln abmelden, ein Passwortwechsel meldet alle anderen Geräte ab, das Konto kann samt allen Serverdaten gelöscht werden. Die Web-Version synchronisiert genauso; Sitzung und Tresorschlüssel liegen auf dem Desktop im Schlüsselbund des Systems. Unterlagen: `docs/SYNC.md`.
- **Add-on-Store: Erweiterungen aus GitHub, ohne FaNotes-Update.** Ein neuer Add-on-Store (Strg+P → „Add-on-Store“ oder Einstellungen → Add-ons) liest das offene Verzeichnis `Nikoheld/FaNotes-Addons`: Wer ein Add-on beisteuern will, legt dort einen Ordner mit `manifest.json`, `main.js` und `README.md` an und öffnet einen Pull Request — nach dem Merge baut das Repository seinen Index selbst neu, und das Add-on erscheint bei allen Nutzern im Reiter „Entdecken“ mit Suche, Kategorien, README, Berechtigungen und Prüfsumme. Installierte Add-ons lassen sich an- und abschalten, neu starten, aktualisieren (auf Wunsch automatisch beim Start) und entfernen; ein Reiter „Entwickeln“ lädt ein eigenes Add-on aus lokalen Dateien und zeigt sein Protokoll. Add-ons erreichen FaNotes über ein einfaches `fanotes`-Objekt: Notizen lesen, schreiben, anlegen und suchen, den Editor steuern, Handschrift und Seitenstatistik lesen, Befehle in die Palette hängen, Panels aus Bausteinen (Überschrift, Text, Markdown, Knopf, Eingabe, Auswahl, Liste, Fortschritt) im neuen Dock rechts zeigen, Einträge in die Statusleiste setzen, Hinweise und Dialoge öffnen, auf Ereignisse wie Notizwechsel, Speichern oder Stiftstriche reagieren, eigene Daten ablegen und — mit eigener Berechtigung und Host-Liste — https-Anfragen stellen. Jedes Add-on läuft in einem eigenen Web Worker ohne Zugriff auf Oberfläche, Dateien oder Netz; jeder Aufruf wird gegen die Berechtigungen aus dem Manifest geprüft, begrenzt (240 Aufrufe je Sekunde, 30 Sekunden je Aufruf) und überwacht: ein Add-on, das hängt, acht Fehler in einer Minute macht oder FaNotes flutet, wird gestoppt und deaktiviert — FaNotes selbst läuft weiter. Die vollständige Dokumentation mit Beispielen liegt im Add-on-Repository.
- **Geteilte Ansicht: jede Seite hat ihre eigene Kamera.** Bisher teilten sich beide Spalten einen Zoom — wer links hineinzoomte, zoomte rechts mit, und das Öffnen der zweiten Spalte setzte den Zoom der ersten zurück. Jetzt zoomt Strg+Mausrad nur das Blatt unter dem Zeiger; Strg++ / Strg+- / Strg+0, Esc und die Trackpad-Geste gelten für die zuletzt berührte Spalte. Die Handschrift-Vorschau der zweiten Spalte wird beim Zoomen in der neuen Größe nachgezeichnet statt als 1×-Bild gestreckt, und die Spaltenbreiten und der Trenner liegen auf ganzen Gerätepixeln, damit der Text der zweiten Spalte nicht auf einem halben Pixel weich gezeichnet wird.
- **Der Aufklapp-Pfeil neben Überschriften sitzt wieder richtig.** Die Falt-Spalte links vom Text war um einen Absatz nach unten verschoben (ein doppelter Innenabstand): Der Pfeil erschien unter der Überschrift statt daneben, und ein Klick traf die falsche Zeile — deshalb tat er nichts. Der Pfeil steht jetzt neben seiner Überschrift, klappt den Abschnitt ein und aus, und der weiße Streifen ohne Punkte, den die Spalte über das Raster legte, ist weg.
...
- **Zoom ohne Ruckeln:** Ein Mausrad- oder Trackpad-Zoom kommt als Schub vieler kleiner Schritte. Bisher baute jeder Schritt die komplette Stiftleiste samt Werkzeugen neu auf und zeichnete die ganze Handschrift in eine zoomgroße Fläche — auf einer vollen Seite mehrfach pro Schub. Jetzt folgt die Stiftebene der Kamera direkt, ohne Neuaufbau, und schärft die Tinte einmal nach, wenn der Schub abgeklungen ist; die Zwischenschritte skaliert das Blatt selbst. Auf einer Seite mit 200 Strichen sinkt die Skriptzeit eines Zoomschubs um rund drei Viertel, die Bildfolge bleibt bei 60 Bildern pro Sekunde.

## 2026.9.21

- **Handschrift und Text können nicht mehr verloren gehen:** Ein Strich, der während des Speicherns gezogen wurde, fiel bisher aus der Seite, weil das Speichern die Seite danach als sauber markierte — jetzt wird so lange nachgeschrieben, bis eine Speicherung den letzten Stand gesehen hat, und ein Stift, der beim Notizwechsel oder Schließen noch auf dem Blatt liegt, wird vorher als Strich übernommen. Stilles Speichern im Hintergrund meldet Fehler sichtbar und versucht es erneut; ein verlorener Grafikkontext malt die Tinte aus dem Modell neu statt ein leeres Blatt zu zeigen. Text- und Tintenspeicherung schreiben die `.famd`-Datei nacheinander statt gleichzeitig, sodass die eine nicht mehr den Stand der anderen überschreibt, und eine fehlgeschlagene Tinten-Einbettung gilt als Fehler, damit die Seite ungespeichert bleibt. Getippter Text wird unter der Notiz verbucht, in der er getippt wurde — auch wenn du sofort danach die Notiz wechselst — und der Editor behält noch nicht gesicherte Tastenanschläge, statt sie durch einen veralteten Stand zu ersetzen. Die Web-Version fragt vor dem Verlassen der Seite, wenn etwas ungespeichert ist.
- **Geteilte Ansicht:** Zwei Notizen nebeneinander oder untereinander. Strg+\ öffnet die zweite Spalte mit der zuletzt besuchten anderen Notiz (oder schließt sie), Umschalt+Klick oder Umschalt+Enter in der Dateileiste, im Tab-Menü und im Schnellwechsel öffnen eine Notiz rechts. Der Trenner lässt sich ziehen (Doppelklick: 50/50, Pfeiltasten: in Schritten, Pos1/Ende: Anschlag, Enter: Ausrichtung wechseln), die Kopfzeile der zweiten Notiz tauscht die Seiten (Strg+Umschalt+\), stellt um auf untereinander, holt die Notiz als einzige in den Vordergrund oder schließt die Teilung. Die zweite Spalte zeigt die gespeicherte Handschrift der Notiz deckungsgleich zum Text; die Spalte mit dem Fokus ist markiert. Umbenennen, Verschieben und Löschen folgen der zweiten Notiz, und Aufteilung, Ausrichtung und Verhältnis bleiben über Neustarts erhalten.
- **Navigation wie im Browser:** Ein Verlauf der besuchten Notizen mit Zurück und Vorwärts (Alt+← / Alt+→, die Daumentasten der Maus, zwei Knöpfe in der Tab-Leiste) ersetzt den bisherigen Zurück-Knopf, der nur nach einem Link funktionierte. Strg+1 … Strg+8 wechseln zum Tab an dieser Stelle, Strg+9 zum letzten, Strg+Umschalt+T öffnet den zuletzt geschlossenen Tab wieder. Tabs lassen sich mit der Maus umsortieren und anheften; angeheftete Tabs bleiben vorn, schmal und ohne Schließen-Kreuz. Ein Rechtsklick auf einen Tab bietet Rechts öffnen, Anheften, Schließen, Andere schließen, Rechts davon schließen, Pfad kopieren und Im Dateimanager zeigen. Offene Tabs, ihre Reihenfolge, angeheftete Tabs, die aktive Notiz und die Teilung kommen nach einem Neustart zurück — auch beim allerersten Start nach der Einrichtung.
- **Neue Zeile bleibt neue Zeile:** Wer im Tastaturmodus mit Enter eine leere Zeile begann, landete nach dem automatischen Speichern wieder am Ende der letzten Zeile mit Text — das Speichern schrieb den am Ende gekürzten Text in den Editor zurück, und der Cursor wurde auf die kürzere Länge gestutzt. Der Editor behält jetzt den Text genau so, wie er getippt wurde, und wenn eine Notiz von aussen ersetzt wird, wird nur die geänderte Stelle ausgetauscht, sodass der Cursor mitwandert statt zu springen.
- **Autokorrektur und Vorschläge:** Die Rechtschreibprüfung unterstreicht nicht mehr nur, sie hilft. Ein eindeutiger Tippfehler wird ersetzt, sobald du das Wort mit Leerzeichen, Satzzeichen oder Enter abschliesst (teh → the, Hasu → Haus) — vertauschte, doppelte oder fehlende Buchstaben, Nachbartasten auf der QWERTZ-Tastatur und vergessene Umlaute; ein blosser Buchstabentausch, ein Wort mit zwei ähnlich guten Kandidaten, Namen, Abkürzungen, camelCase, Code, Formeln, Links und Tags bleiben unangetastet. Das korrigierte Wort ist kurz gepunktet unterstrichen; Rückschritt direkt danach stellt dein Wort wieder her und merkt sich, dass du es so willst, Strg+Z nimmt genau die Korrektur zurück. Rechtsklick oder Strg+. auf ein unterstrichenes Wort öffnet Vorschläge (auch für zusammengesetzte Wörter und ein vergessenes Leerzeichen wie „auchdas“), „Ignorieren“ für diese Sitzung und „Zum Wörterbuch hinzufügen“ — Wörter im eigenen Wörterbuch werden nie mehr unterstrichen. Die Autokorrektur lässt sich in den Einstellungen unter „Editor“ abschalten; die Wortlisten laden erst, wenn die Notiz offen ist, damit der Start gleich schnell bleibt.
- **Dateileiste, Schnellwechsel und Pfadleiste:** In der Dateileiste bewegen ↑/↓ durch die sichtbaren Zeilen, Pos1/Ende springen an Anfang und Ende, ← und → klappen Ordner zu und auf oder wechseln zur Elternzeile, Entf löscht den markierten Eintrag (mit Rückfrage), und die aktive Notiz wird beim Öffnen in den sichtbaren Bereich gerollt. Aufgeklappte Ordner bleiben je Tresor gemerkt. Strg+O öffnet den Schnellwechsel: Tippen filtert alle Notizen unscharf nach Titel und Pfad, zuletzt geöffnete stehen vorn, Enter öffnet, Umschalt+Enter öffnet rechts. Die Statusleiste zeigt den Pfad der aktiven Notiz als Brotkrumen; ein Klick auf einen Ordner zeigt ihn in der Dateileiste.
- **Update sagt, was das Schul-WLAN blockiert:** Scheitert die Prüfung oder der Download, schreibt FaNotes den Vorgang ins Update-Protokoll (`updates/download-failures.log`) und zeigt unter Einstellungen → Updates, was dazwischenliegt: Umleitung auf eine Anmeldeseite, HTTP-Filter, ersetzte HTML-Sperrseite, HTTPS-Proxy mit fremdem Zertifikat, DNS ins Schulnetz, abgebrochene Verbindung oder Zeitüberschreitung.

## 2026.9.20

- **Lineal, Geodreieck und Zirkel wie auf dem Schreibtisch:** Die drei Zeichenhilfen im Stiftmodus sind neu gebaut und rechnen in echten Millimetern des Blatts. Das Lineal hat jede Länge von 5 bis 50 cm (Griff am linken Ende, Feld oder Vorgaben), trägt eine Zentimeter- oder Zoll-Skala und zeichnet an beiden Kanten; während du ziehst, liest eine kleine Blase Länge und Winkel der Kante ab. Das Geodreieck ist ein echtes Geodreieck: Die lange Kante ist die Zeichenkante mit Skala ab der Mitte (links negativ, rechts positiv), am Mittelpunkt sitzt der doppelte Winkelmesser 0–180°, die beiden Schenkel zeichnen im 45°-Winkel, im Dreieck liegen Parallelen-Hilfslinien, und mit F (oder dem Griff) klappt die Spitze auf die andere Seite. In der Nähe des Lineals legt es sich von selbst mit der Kante bündig oder mit einem Schenkel senkrecht an und gleitet daran entlang — Parallelen und Senkrechte wie mit Lineal und Dreieck auf Papier (Alt beim Ziehen: nicht anlegen). Der Zirkel rastet mit der Nadel an Lineal und Geodreieck ein, misst den Radius am gelben Punkt auch an Kanten ab („Maß übertragen“ mit gesperrtem Radius), zeichnet am grünen Punkt Bögen im Winkelraster oder per Taste 30°, 45°, 60°, 90°, 180°, 270° und ganze Kreise, im oder gegen den Uhrzeigersinn, optional mit einem Kreuz im Mittelpunkt — und ein Bogen bleibt auch dann rund, wenn das Blatt erst für ihn wachsen muss.
- **Zeichenhilfen mit Optionen:** Ein Feld „Optionen“ in der Stiftleiste öffnet die Karte „Zeichenhilfen“: Einheit (cm oder Zoll), Winkelraster (frei, 1°, 5°, 15°, 45°), Magnetstärke (aus, schwach, normal, stark), durchsichtige Werkzeuge, und je Werkzeug Länge, Größe, Winkel, Radius, Radius sperren, Mine-Stellung, Spiegeln, Parallelen, Anlegen, Mittelpunkt markieren, Bogenrichtung sowie Fixieren, In die Blattmitte holen und Ausblenden. Zahlen kannst du tippen oder mit Vorgaben setzen; die Einstellungen bleiben über Neustarts erhalten, die Werkzeuge liegen dort, wo du sie in dieser Sitzung abgelegt hast. Pfeiltasten verschieben das zuletzt berührte Werkzeug um 1 mm (Umschalt: 10 mm), Q/E drehen es. Der Stift zeichnet auch dann an der Kante, wenn er bis 6 mm innerhalb des Werkzeugkörpers aufsetzt; weiter innen verschiebt er es. Die Werkzeuge waren auf nicht quadratischen Blättern verzerrt gedreht und ihre Fangkanten lagen neben der gezeichneten Kante — die Drehung rechnet jetzt in Millimetern, die Werkzeugebene deckt sich mit dem Blatt statt mit dem Kamerarand, und ein neues Werkzeug erscheint immer ganz auf dem Blatt. Alle Beschriftungen, Blasen und Meldungen gibt es auch auf Englisch.
...
- reine Buchstabenwörter werden nicht mehr wegen einzelner höher oder tiefer geschriebener Zeichen als Indexketten ausgegeben: Ein vollständig erkanntes Wörterbuchwort sowie klar grossgeschriebene Eigennamen wie `Hallo` und `Fabio` überstimmen eine ansonsten strukturlose `H_{allo}`-/`F_{abio}`-Hypothese auch bei deutlich unruhiger Grundlinie; echte Ziffernindexe (`x_1`), unbekannte klar versetzte Buchstabenindexe (`x_{max}`), Brüche, Relationen und Operatoren bleiben mathematisch
- das kontextuelle Zeilenmodell verwendet jetzt echte Zwei-Strahl-Dekodierung statt nur eines gierigen Pfades; die vorhandene konservative visuelle Rangfolge übernimmt ausschließlich ausreichend gestützte Modellkandidaten. Auf 120 unabhängigen deutschen ScaDS-Zeilen sinkt die CER nach der vollständigen Wortprüfung von 10,27 auf 10,08 Prozent (7 verbessert, 0 verschlechtert), auf 120 englischen IAM-Zeilen von 5,66 auf 5,49 Prozent (4 verbessert, 0 verschlechtert); gerade und ungerade Prüfhälften profitieren jeweils, während Strahl drei und vier keinen weiteren Treffer liefern
- der echte Web-Worker-Audit hält mit zwei Strahlen alle sieben kontrollierten verbundenen Zeilen, die zweizeilige Folge und die ungesehene IAM-OnDB-Zeile exakt; mit 2,62 Sekunden Kaltstart und rund 0,66 Sekunden warmem Mittel bleibt der zusätzliche Kandidat innerhalb der bisherigen Laufzeit, ohne Modellkopie oder parallelen Inferenzlauf
- erfundene Leerzeichen in unbekannten Namen und Fachwörtern werden jetzt direkt an der Stifttrajektorie geprüft: Kreuzt ein durchgehender, lokal dichter Stiftzug genau die vom Zeilenmodell vorgeschlagene Grenze, wird sie auch ohne Wörterbucheintrag entfernt (`Xyq aro` → `Xyqaro`); zwei lediglich eng nebeneinanderliegende unabhängige Stiftkörper und echte breite Wortlücken bleiben getrennt
- der klassische Abstandsschätzer wertet einen einzelnen abgesetzten Anschluss innerhalb eines Wortes nicht mehr allein wegen seines statistischen Ausreisserstatus als Wortgrenze; eine strengere absolute Schwelle aus Buchstabenbreite und x-Höhe schützt unbekannte Wörter, während kompakte Grenzen zwischen zwei bekannten Wörtern weiterhin durch gemeinsame Geometrie und Wortpartitionierung zurückgewonnen werden
- bei einem seltenen gültigen Einzelwort, das genau einen gleich langen häufigen Ein-Zeichen-Nachbarn besitzt, entscheidet FaNotes nicht mehr allein nach Sprachhäufigkeit: Nur wenn der kompakte Zeichenpfad den Nachbarn mindestens gleich gut stützt, wird eine zweite engere und dünnere Darstellung visuell geprüft; stimmen beide Ansichten überein, wird etwa `computed` sicher zu dem tatsächlich geschriebenen `computer`, während übereinstimmende Wörter, mehrdeutige Kandidaten und grossgeschriebene Namen keinen Zusatzlauf auslösen
- der verpflichtende Browseraudit verlangt jetzt nicht mehr nur vier Treffer, sondern alle sieben kontrollierten verbundenen Zeilen exakt: `test`, `hallo`, `lernen`, `mathe`, `computer`, `hallo test` und `computer test`; die zweizeilige Folge `hallo test`/`lernen macht` sowie die ungesehene IAM-OnDB-Zeile mit 0 Prozent CER bleiben ebenfalls exakt
- vorzeitig beendete Kontextmodelle können den sichtbaren Zeilenrest jetzt auch dann nicht mehr löschen, wenn einzelne Wortcrops fehlen: Eine längere rohe und korrigierte CTC-Folge muss denselben mindestens zwei Wörter langen Präfix stützen, der Rest muss positionsgetreu übereinstimmen und ein echtes Satzende sperrt jede Verlängerung; klare Schreibzeilen bleiben bis 24 statt bisher 12 physische Wortgruppen segmentiert
- die Wortfusion nutzt nun die tatsächlich gemessene Zeichenanzahl bei Wörterbuchkonflikten: ein anders langer seltener Nachbar blockiert die einzige gleich lange häufige Ein-Zeichen-Korrektur nicht mehr (`mallo` → `hallo`), und zwei unabhängig an verschiedenen Positionen beschädigte Modellwörter dürfen genau einen gemeinsamen visuellen 2+2-Mittelpunkt bilden (`rennen`/`learner` → `lernen`); gültige Wörter, Namen und mehrdeutige Fachformen bleiben geschützt
- im echten Browserpfad werden dadurch `hallo`, `hallo test` und die zwei Zeilen `hallo test`/`lernen macht` vollständig erkannt; die ungesehene IAM-OnDB-Zeile bleibt bei 0 Prozent CER. Auf je 120 unabhängigen Modellzeilen verbessert sich der deutsche Kontext von 10,46 auf 10,24 Prozent CER bei 11 verbesserten und keiner verschlechterten Zeile, der englische Kontext bleibt bei 5,64 Prozent und ebenfalls null Verschlechterungen
...
- die gemeinsame Text-/Mathematikerkennung verlangt für Hoch- und Tiefstellungen jetzt eine klare vertikale Verschiebung relativ zu Basis und Folgezeichen; natürliche Grundlinienschwankungen und unterschiedlich hohe Buchstaben erzeugen dadurch keine scheinbaren Indexe mehr
- auch kurze bekannte Wörter wie `der` können eine reine falsche Indexhypothese sicher überstimmen, während echte Hoch-/Tiefstellungen, Brüche, Wurzeln sowie Integral-, Summen- und Produktgrenzen mathematisch bleiben
- die erste Sprachkontextstufe respektiert jetzt das vollständige lokale OCR-Wörterbuch; korrekt gelesene Flexionsformen, britische Varianten, Namen und Fachwörter werden nicht länger von einer kleineren Häufigkeitsliste in ähnlich aussehende Alltagswörter umgeschrieben
- die abschliessende Wörterbuchkorrektur prüft Eindeutigkeit über alle Wortlängen sowie unabhängige Zeichen-, Präfix- und Suffixplausibilität; Apostrophe, Bindestriche, kurze Punkt-Abkürzungen und Namen im Satzinneren bleiben geschützt. Auf je 120 unabhängigen englischen und deutschen Zeilen sinkt die CER von 5,70 auf 5,64 beziehungsweise von 10,48 auf 10,27 Prozent, ohne eine einzige kontextbedingt verschlechterte Zeile
- CTC- und Kontextmodell stimmen ihre rohen Zeichenpositionen jetzt vor jeder Wörterbuchumschreibung ab: Ein häufiges Wort wird nur bei eindeutiger gemeinsamer visueller Nähe eingesetzt, während eine seltene Beam-Alternative keinen klareren direkten Zeichenpfad mehr verdrängt; die verbundene Browserzeile `hallo test` und eine ungesehene IAM-Stiftzeile werden dadurch vollständig erkannt
- erkannte Wortabstände werden zusätzlich gegen die tatsächlichen horizontalen Tintenlücken geprüft: Ist selbst die grösste Lücke eindeutig nur ein Buchstabenabstand und die verbundene Form ein gültiges Wort, verschwindet ein erfundener Modellabstand auch dann, wenn beide Fragmente einzeln sinnvoll wären (`will kommen` → `willkommen`); eine echte breite Lücke bleibt unverändert
- umgekehrt werden eindeutig gemessene physische Wortlücken monoton auf die erkannten Zeichenpositionen zurückprojiziert, falls die wortweise Erkennung auf die Gesamtzeile zurückfallen musste; `hallotest` erhält dadurch wieder `hallo test`, zusätzliche Modellgrenzen wie `hal lo test` werden auf die tatsächlich sichtbaren zwei Wörter reduziert und bereits korrekte Abstände nicht verschoben
- vorzeitig beendete Kontextzeilen können keine sicher einzeln erkannten Endwörter mehr löschen: Das Kontextpräfix wird wortweise gegen seine rohe visuelle Ausgabe ausgerichtet, darf lokal gestützte Lesarten wie `dem` → `den` übernehmen und wird danach verlustfrei mit allen fehlenden physischen Wortgruppen ergänzt; ein nicht ausrichtbarer fremder Anfang ersetzt überhaupt nichts
- die gleiche physische Vollständigkeitsprüfung kann bis zu drei eindeutig ungestützte häufige Kontextwörter an beliebigen Zeilenpositionen entfernen: Eine begrenzte Sequenzsuche muss nach dem Entfernen genau eine beste vollständige Ausrichtung zu allen sichtbaren Wortgruppen ergeben. Echte Wortaufteilungen wie `cannot`/`can not`, unbekannte Zusätze und bei Wiederholungen mehrdeutige Positionen werden nie automatisch beschnitten
- die harte geometrische Schwelle für Hoch- und Tiefstellungen toleriert jetzt bis zu 16 Prozent der Basishöhe statt 11 Prozent Grundliniendrift; ein systematischer Sweep hält 336 normale Varianten von `Test`, `Hallo`, `Fabio` und `Lernen` bis ±15 Prozent vollständig indexfrei, während 27 echte Hoch-/Tiefstellungsvarianten ab 18 Prozent samt Potenzen und Indizes erhalten bleiben
