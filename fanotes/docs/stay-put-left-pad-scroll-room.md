...
2026.9.15 fixes the remaining *visual* slip on Linux report `1788704214528` (paint-to-edge autoexpand on `Mathematik/Untitled note.md`, padX 108, height 1408→2448, nine `grew: true`). Paper coordinates were already correct; the composited glyph still slid until a later canvas mutation (erasing leftover ghost ink). The live grow step now runs the same canvas/surface visual update (`refreshPaperCanvasSurface` / `applyVisualGrowCorrection`) and a vsync-class double-rAF follow-up — not a compositor-generation bump alone, and not a battery-draining poll. Gating check: `npm run check:latest-bug`.
