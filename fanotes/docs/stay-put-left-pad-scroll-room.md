...
Gating check: `npm run check:stay-put` drives a closed stay-put set through `applyStayPutOp` / `stayPutPaperMovedByPadOnly`: pan-only, max-edge height and width, min-edge pad, overlay jump, nested editor lock, sheet-origin shift, plus report `1788435936618`. Paper X/Y may change only by a new origin pad. Live grow uses `stayPutAfterExtentGrow` and `pinPaperViewportAfterExtentGrow`.
