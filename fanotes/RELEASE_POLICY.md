...
- `N` is the stable release number within the month and is normally limited to `1`–`4`.
- A fifth or sixth stable (`N` = `5` or `6`) is allowed only for urgent correctness, security or input regressions that cannot wait for the next calendar month.
...
- Soft maximum: four stable releases per calendar month.
- Hard maximum: six stable releases per calendar month.
...
- A fifth or sixth stable release is reserved for urgent correctness, security or input regressions (for example desktop input freezes) that must ship before the next month.
