...
assert.match(worker, /num_beams: 1/u)
assert.match(worker, /num_return_sequences: 1/u)
assert.match(worker, /caller obtains a real alternative from a separately/u)
assert.doesNotMatch(worker, /num_beams: [2-9]/u)
assert.match(worker, /do_sample: false/u)
assert.doesNotMatch(worker, /ForceAlternativeProcessor|AlternativeCaptureProcessor/u)
...
console.log('TrOCR-Integration geprüft: Modell/WASM bytegenau, deterministischer Decoder, Worker-Lazy-Loading, Electron-Protokoll, Isolation und PWA-Startcache.')
