...
void (async () => {
  ...
  try {
    ...
    const noteCountBefore = await cdp.evaluate(`window.fanotes.getTree().then((entries) => {
    ...
    const visibleNoteCreation = await cdp.evaluate(`(async () => {
      ...
      const entries = await window.fanotes.getTree()
      const count = (items) => items.reduce((total, entry) => total + (entry.kind === 'file' ? 1 : count(entry.children || [])), 0)
      return {
        activePath,
        content: await window.fanotes.readFile(activePath),
        noteCount: count(entries),
        errors: [...document.querySelectorAll('.toast.error')].map((node) => node.textContent.trim()),
      }
    })()`)
    if (visibleNoteCreation.noteCount !== noteCountBefore + 1 || !visibleNoteCreation.content.startsWith('# Unbenannte Notiz') || visibleNoteCreation.errors.length) {
      throw new Error(`Notizerstellung über die Oberfläche fehlgeschlagen: ${JSON.stringify(visibleNoteCreation)}`)
    }

    await cdp.evaluate(`window.fanotes.writeFile('Willkommen.md', '# Willkommen bei FaNotes\\n\\n- Unter **Durchkritzel-Empfindlichkeit** stellst du von 0 bis 100 Prozent ein, wie gründlich du dafür kritzeln musst.\\n- Mit \`Strg+Z\` holst du eine versehentliche Löschung sofort zurück.\\n\\nDieser Abschnitt muss hell bleiben.\\n')`)
    await cdp.send('Page.reload', { ignoreCache: true })
    await waitFor(cdp, `Boolean(window.fanotes?.platform === 'web' && document.querySelector('.app-shell'))`, 'FaNotes nach der Welcome.md-Migration')
    const welcomeRepair = await cdp.evaluate(`(async () => {
      const content = await window.fanotes.readFile('Willkommen.md')
    ...
    const core = await cdp.evaluate(`(async () => {
      const created = await window.fanotes.createNote('Mathematik', 'Web Persistenz')
      await window.fanotes.writeFile(created.relativePath, '# Web Persistenz\\n\\nDas ist ein Tset.\\n\\nThis is an exampel.\\n\\nFormel: $x^2 + 2x + 1$\\n')
      const search = await window.fanotes.search('Persistenz')
      ...
      const drawing = await window.fanotes.saveDrawing({ title: 'Web Tinte', drawingJson })
      const loadedDrawing = await window.fanotes.readDrawing(drawing.id)
      return { platform: window.fanotes.platform, path: created.relativePath, content: await window.fanotes.readFile(created.relativePath), hits: search.length, transcript: JSON.parse(loadedDrawing.drawingJson).searchTranscript }
    })()`)
    ...
    await cdp.send('Runtime.evaluate', { expression: `window.__worksheetPromise = window.fanotes.importWorksheet().then((value) => window.__worksheet = value)`, awaitPromise: false })
    ...
    const worksheet = await cdp.evaluate(`(async () => ({ source: await window.fanotes.readAssetDataUrl(window.__worksheet.sourceRelativePath), stored: (await window.fanotes.readWorksheet(window.__worksheet.id)).id === window.__worksheet.id }))()`)
    ...
    await cdp.send('Page.reload', { ignoreCache: true })
    await waitFor(cdp, `Boolean(window.fanotes?.platform === 'web' && document.querySelector('.app-shell'))`, 'FaNotes nach dem Neuladen')
    const persisted = await cdp.evaluate(`window.fanotes.readFile('Mathematik/Web Persistenz.md')`)
    ...
    const backupUi = await cdp.evaluate(`(async () => ({
      ...
      supported: (await window.fanotes.getServerBackupState()).supported,
    }))()`)
    ...
    if (backupEnrollmentCode) {
      const activated = await cdp.evaluate(`(async () => {
        const state = await window.fanotes.enableServerBackup(${JSON.stringify(backupEnrollmentCode)})
      ...
      const deleted = await cdp.evaluate(`window.fanotes.deleteServerBackup()`)
    ...
    await cdp.send('Page.reload', { ignoreCache: true })
    await waitFor(cdp, `Boolean(window.fanotes?.platform === 'web' && document.querySelector('.markdown-editor .cm-content'))`, 'Der vollständige Editor im Offline-Modus')
    const offlineContent = await cdp.evaluate(`window.fanotes.readFile('Mathematik/Web Persistenz.md')`)
