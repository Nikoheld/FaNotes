...
void (async () => {
  ...
  try {
    ...
    const spellingResources = await cdp.evaluate(`window.fanotes.loadSpellingResources()
    ...
    await waitFor(cdp, `window.fanotes.listDrawings().then((items) => items.length > 0)`, 'Die automatische Speicherung der Zeichnung')
    const savedArt = await cdp.evaluate(`(async () => {
      const drawings = await window.fanotes.listDrawings()
      const loaded = await window.fanotes.readDrawing(drawings[0].id)
    ...
    await waitFor(cdp, `window.fanotes.listDrawings().then(async (items) => {
      const loaded = await window.fanotes.readDrawing(items[0].id)
