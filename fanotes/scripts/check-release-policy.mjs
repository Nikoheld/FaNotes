...
const match = /^(\d{4})\.(\d{1,2})\.([1-9])(?:-beta\.([1-9]\d*))?$/u.exec(version)
if (!match) throw new Error('Calendar versions must use YYYY.M.N or YYYY.M.N-beta.B; stable N is limited to 1–9.')
...
const stable = monthly.filter((release) => /^v\d{4}\.\d{1,2}\.[1-9]$/u.test(release.tag_name))
if (betaNumber === null) {
  if (stable.length >= 9) throw new Error('Nine stable releases already exist this month; wait for the next calendar month.')
  ...
  const targetedStableNumber = Math.min(9, stable.length + 1)
