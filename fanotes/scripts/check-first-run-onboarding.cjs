...
assert.ok(names.includes('Französisch'), 'Französisch fehlt in der Fächerauswahl.')
...
assert.match(renderer, /first-run-preview-stage/u, 'Die getrennte Schwebestufe der Vorschau fehlt.')
assert.match(renderer, /pathLength="1"/u, 'Die Tintenzeichnung hat keine normierte Pfadlänge.')
assert.match(renderer, /Französisch/u, 'Französisch fehlt in der Einrichtungsvorschau.')
