...
try {
  ...
  if (process.env.FANOTES_NEURAL_DEBUG === '1') console.log(JSON.stringify(result, null, 2))
  ...
  assert.equal(
    result.iamPhysicalWordGroups,
    5,
    `Die klar getrennten Wörter der echten IAM-Zeile wurden geometrisch nicht vollständig erfasst: ${JSON.stringify(result)}`,
  )
  assert.equal(
    result.iamOnline.recognized,
    result.iamOnline.expected,
    `Die echte, bisher ungesehene IAM-OnDB-Stiftzeile muss vollständig stimmen: ${JSON.stringify({ ...result.iamOnline, iamCharacterErrorRate })}`,
  )
  ...
  assert.equal(
    result.words.find((entry) => entry.expected === 'hallo')?.recognized,
    'hallo',
    `Ein unsinniges gleich langes Modellwort muss zur eindeutigen häufigen Wortform korrigiert werden: ${JSON.stringify(result.words)}`,
  )
  assert.equal(
    result.words.find((entry) => entry.expected === 'computer')?.recognized,
    'computer',
    `Zwei gültige Nachbarwörter müssen über eine unabhängige visuelle Zweitansicht auseinandergehalten werden: ${JSON.stringify(result.words)}`,
  )
  const exactControlledWords = result.words.filter((entry) => entry.recognized === entry.expected)
  assert.equal(
    exactControlledWords.length,
    result.words.length,
    `Jede kontrollierte verbundene Textzeile muss exakt sein: ${JSON.stringify(result.words)}`,
  )
  assert.ok(
    result.words.every((entry) => (
      editDistance(entry.recognized, entry.expected) <= Math.max(1, Math.ceil(entry.expected.length * 0.18))
    )),
    `Eine kontrollierte Form weicht visuell zu weit vom geschriebenen Text ab: ${JSON.stringify(result.words)}`,
  )
  assert.ok(
  ...
  assert.equal(
    result.multiline,
    'hallo test\nlernen macht',
    `Unabhängig beschädigte Wortpfade müssen auch innerhalb mehrerer Schreibzeilen vollständig fusioniert werden: ${JSON.stringify(result.multilineLines)}`,
  )
