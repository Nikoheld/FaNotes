...
const run = async () => {
  ...
  for (const item of cases) {
    ...
    words.push({
      ...
      beam: result.lines.map((line) => line.beamText ?? '').join('\n').toLocaleLowerCase(item.language),
      greedy: result.lines.map((line) => line.greedyText ?? '').join('\n').toLocaleLowerCase(item.language),
      alternatives: result.lines.flatMap((line) => line.alternatives ?? []).map((entry) => entry.toLocaleLowerCase(item.language)),
  ...
  return {
    ...
    multilineLines: multiline.lines.map((line) => ({
      text: line.text,
      rawText: line.rawText,
      beamText: line.beamText,
      greedyText: line.greedyText,
    ...
    iamOnline: {
      ...
      alternatives: iamOnline.lines[0]?.alternatives,
    ...
    iamPhysicalWordGroups: neuralTextPhysicalWordGroupsForTests(
      iamOnlineStrokes(3.7),
      iamOnlineFixture.sourceWidth,
      iamOnlineFixture.sourceHeight,
    )[0]?.length ?? 0,
    ...
    repairedWords: [
      applyNeuralWordContext('dsa', 'de'),
      applyNeuralWordContext('wehn', 'en'),
