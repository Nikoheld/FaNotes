...
export const runUjiPersonalizedLineAudit = async (records: UjiRecord[]) => {
  ...
  const includeBlindRasterDiagnostics = parameters.get('blind-raster') === '1'
  const expectedCountDiagnosticsOnly = parameters.get('diagnostic-count') === 'expected'
  const rawDiagnosticSegmentationIndex = parameters.get('diagnostic-index')
  const requestedDiagnosticSegmentationIndex = Number(rawDiagnosticSegmentationIndex)
  const diagnosticSegmentationIndex = rawDiagnosticSegmentationIndex !== null
    && Number.isInteger(requestedDiagnosticSegmentationIndex)
  ...
  for (let writerIndex = 0; writerIndex < writers.length; writerIndex += 1) {
    ...
    for (const expected of expectedWords) {
      ...
      // Match the same coordinate space used by both the neural renderer and
      // the embedded GlyphenWerk workspace. Falling back to the note-canvas
      // default height here changed the rendered pen width by more than 2x,
      // so the audit was no longer exercising the pixels seen in production.
      const personalized = await recognizePersonalizedTextLine(
        strokes, resources, neural, 'de', true, 900, 560,
      )
      ...
      const blindRaster = fusedDistance > 0 && includeDiagnostics && includeBlindRasterDiagnostics
        ? await (async () => {
            const rendered = renderPersonalRasterLine(strokes, 900, 560)
            if (!rendered) return null
            const measured = estimatePenLiftTextCharacterCount(strokes)
            const visibleTokens = personalized.tokens.filter((token) => !token.isLayout).length
            const result = await recognizePersonalRasterPixels(
              rendered,
              personalSamples,
              '',
              'de',
              [measured, visibleTokens].filter((count): count is number => Boolean(count)),
            )
            return result ? {
              prediction: result.prediction,
              columnBandCount: result.columnBandCount,
              candidates: result.candidates.slice(0, 8).map((candidate) => ({
                targetCount: candidate.targetCount,
                text: candidate.text,
                visualText: candidate.visualText,
                score: candidate.score,
                averageVisualCost: candidate.averageVisualCost,
                cutInk: candidate.cutInk,
                beams: candidate.sequenceBeams.slice(0, 4),
              })),
            } : null
          })()
        : null
      ...
      cases.push({
        ...
        expected,
        neural: neuralText,
        neuralLines: includeDiagnostics ? neural.lines.map((line) => ({
          text: line.text,
        ...
        rasterDecision: personalized.rasterDecision,
        blindRaster,
