...
const runOnce = () => {
  ...
  const sheet = { x: 0, y: 0, width: 900, height: 1273 }
  ...
  assert.equal(fill.x, plane.x)
  assert.equal(fill.y, plane.y)
  assert.ok(fill.width >= plane.width)
  ...
  const extraPaper = { x: layout.plane.width - 20, y: 20 }
  assert.equal(paperRulingContinuousGrid(
    { x: layout.sheet.x + 14, y: layout.sheet.y + 14 },
    extraPaper,
    layout.origin,
    layout.tile,
  ), true)
