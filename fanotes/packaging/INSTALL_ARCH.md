...
```bash
chmod +x FaNotes-2026.9.20-x86_64.AppImage
./FaNotes-2026.9.20-x86_64.AppImage
```
...
install -m755 FaNotes-2026.9.20-x86_64.AppImage \
...
```bash
./FaNotes-2026.9.20-x86_64.AppImage --appimage-extract-and-run
```
...
tar -xzf FaNotes-2026.9.20-x86_64.tar.gz -C "$HOME/.local/opt"
ln -sfn \
  "$HOME/.local/opt/FaNotes-2026.9.20-x86_64/fanotes" \
  "$HOME/.local/bin/fanotes"
"$HOME/.local/bin/fanotes"
```
...
rm -rf "$HOME/.local/opt/FaNotes-2026.9.20-x86_64"
```
...
1. `FaNotes-2026.9.20-x86_64.tar.gz`, `PKGBUILD`, `fanotes.desktop`, `fanotes.svg` und `LICENSE` gemeinsam in einen leeren Arbeitsordner kopieren.
